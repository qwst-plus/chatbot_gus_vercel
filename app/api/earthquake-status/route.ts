...
import { requireAsahikawaGasAdmin, requireQuest } from "@/lib/credentials";
...
// 認証は以下のいずれか: ① ADMIN_SECRET（外部/API利用）② クウェスト社内アカウントまたは
// 旭川ガス側の管理者アカウントのログインセッション（運用ダッシュボードの手動操作ボタンから、
// ADMIN_SECRETをブラウザに露出させずに呼べるようにするため）
export async function PATCH(req: NextRequest) {
  ...
  if (!hasValidSecret) {
    ...
    if (session) {
      isAuthorizedUser =
        !!(await requireQuest(session.userId)) ||
        !!(await requireAsahikawaGasAdmin(session.userId));
    }
  }

  if (!hasValidSecret && !isAuthorizedUser) {
